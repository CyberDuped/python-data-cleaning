...
# Clean Text Fields
df["business_name"] = df["business_name"].str.title().str.strip()
df["business_address"] =df["business_address"].str.strip()

# Validate Key Fields
df = df[df["inspection_score"].between(0,100)]

df.to_csv(r"..\data\cleaned\Health_Inspection_Scores_2016-2019_cleaned.csv", index=False)
